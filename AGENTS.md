...
- **Usage:**
  - Reference tokens through CSS variables (e.g., `var(--primary)`, `var(--border-radius-10)`, `var(--shadow-m)`).
...
- **Component layout:**
  ````
  core/{atoms|molecules|organisms}/ComponentName/
    ├── index.tsx            # component
    ├── index.type.tsx       # props/types
    ├── __tests__/           # unit tests
    └── __stories__/variants # Storybook stories
  ````
- **Types:** Props interfaces named `[ComponentName]Props` extending `BaseProps`. Capitalize type aliases (e.g., `Appearance`, `Size`).
- **Exports:** Named export of component from `index.tsx`; export props from `index.type.tsx`.
